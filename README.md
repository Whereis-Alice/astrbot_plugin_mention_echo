...
[![version](https://img.shields.io/badge/version-v1.1.0-1e6fff?style=flat-square)](https://github.com/Whereis-Alice/astrbot_plugin_mention_echo/releases)
...
- **这类插件跑久了会把硬盘吃光。**
  ...
  一条带图记录 931 字节，加上默认开启的自动清理与三层配额，见 [存储占用与自动清理](#存储占用与自动清理)。
...
| 能力 | 说明 |
| --- | --- |
| 🪶 **真的轻量** | 数据库里只存引用，图片字节只落磁盘。一条带图记录 **931 字节**，一个攒满 300 条的会话全读进内存也只要 **0.67 MB**。同样场景下上游会占 76 MB。 |
...
| 🕒 **一条命令两种用法** | `谁艾特我` 看全部；`谁艾特我 3` 只看最近 3 次并附一行摘要，久没看群补课用它。要不要带上当时的群聊上下文，配置里一键切换。 |
...
| `谁艾特我 3` `谁艾特我3次` `谁艾特我 12 条` | 只看最近 N 次艾特（N 可填 1–50），久没看群补课用这个 | 所有人 | 图片 + 一行摘要 |
...
| | 输出 | 开头那句话 |
| --- | --- | --- |
| `谁艾特我` | 存着的记录全部出图，多了自动分页 | 先回一句「稍等」，再发图 |
| `谁艾特我 3` | 只有最近 3 次（可填 1–50） | 换成一行摘要，和图片一起发出来 |

摘要写的是：这次看了几条、一共存了多少条、最近一次是多久以前（`8 小时前`、`3 天前（09-03 21:40）`）。
...
| 填 | 效果 | 适合 |
| --- | --- | --- |
| `跟随群设置`（默认） | 只有群里发过 `开启艾特上下文` 才展示 | 大多数群 |
| `始终展示` | 只要记录里存着上下文就展示 | 你就是那个久不看群的人 |
| `从不展示` | 只看被 @ 的那一条，出图最小 | 只想知道「谁找我」 |

几件需要知道的事：

- 上下文得**先存下来才看得到**。群里发过一次 `开启艾特上下文`（群管 / 主人权限）之后的艾特才带前后文，改配置**不能把历史补回来**。真没存上，摘要里会直接告诉你怎么开。
- 前后各取几条由 `record.查询上下文条数` 决定，默认前后各 5 条。
- 想让 `谁艾特我` 默认就是补课视图，把 `record.查询默认只看最近多少次艾特` 设成 3。命令里写的数字优先级更高。
- 上面这两项**只影响你主动查询**。回群时的自动提醒仍然跟随群设置，不会因为你改了查询配置，提醒就开始给你贴一大段群聊记录。
...
| `mecache://2026/09/06/xxxx.jpg` | 指向本地缓存文件的相对路径 |
...
一条典型记录（1 张 300 KB 图 + 5 条上下文）：

| | 本插件 | 上游同场景 |
| --- | --- | --- |
| 进数据库的体积 | **931 字节** | 801,740 字节 |
| 常驻内存 | **3.2 KB** | —— |

纯文字记录更小：入库 845 字节，内存 2.9 KB。

一个攒满配额的会话（300 条，其中约三分之一带图）：
...
| 全部读进内存 | **0.67 MB** | —— |
| 读取过程内存峰值 | 2.2 MB | —— |
| 一次查询（渲染最近 10 条）临时峰值 | **15.8 KB** | —— |

磁盘那边只剩原图文件，上限是「最近 20 条 × 单图大小」/ 会话，再由 `message_images 128 MB` 和总配额 256 MB 兜底。

插件的运行时缓存也都有硬上限：维护锁 512 项（≈100 KB）、群活跃度表 4096 项（≈480 KB）、成员昵称缓存 300 秒 TTL。**不会随运行时间无限增长。**

> 以上数字来自本机实测（`tracemalloc` + 真实入库路径）。`tests/test_storage.py` 里有对应的回归用例，保证这些边界不会被后续改动破坏。
...
| 单次查询最多几页 | 0 | 0 = 不限制 |
| 查询上下文条数 | 5 | 前后各多少条 |
| 查询时是否连带艾特前后的群聊记录 | 跟随群设置 | `跟随群设置` = 只有开过 `开启艾特上下文` 的群才展示；`始终展示` = 存了就展示；`从不展示` = 只看被 @ 的那一条。**只影响查询**，回群提醒不受影响 |
| 查询默认只看最近多少次艾特 | 0 | 0 = 全部记录（分页）。设成 3 就等于 `谁艾特我` 默认只看最近 3 次；命令里写的数字优先 |
...
- 两个测试都只用标准库、全程不联网，`python tests/xxx.py` 直接跑，不需要装 AstrBot 或 pytest。
