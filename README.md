...
- **久没看群，想一口气把最近几次 @ 都补上。**
  `艾特回顾` 会把最近 1–5 次 @ 你的消息，连同**当时那段群聊上下文**一起还原出来 —— 不只是 @ 你的那一句，而是那一段对话。
- **这类插件跑久了会把硬盘吃光。**
...
| 🕒 **艾特回顾** | 久没看群，`艾特回顾 3` 一次看完最近 3 次 @，每次都带上当时的群聊上下文，不用自己往上翻。 |
...
| `艾特回顾` `艾特回顾 3` `艾特补课` `at_recap` `catch_up` | 久没看群时补课：最近 N 次艾特 **+ 每次的群聊上下文**（N 可填 1–5，默认 3） | 所有人 | 图片 |
...
`谁艾特我` 给的是「谁 @ 过我」的清单。可如果你已经好几天没进群，光看被 @ 的那一句，常常看不懂在讲什么。这种时候用：
...
它和 `谁艾特我` 只差一点，但这一点很关键：**回顾会强制带上每次艾特前后的群聊记录**，把那一段对话原样还原，而不是只给你被 @ 的那一句。

- 次数可填 **1–5**，指的是「最近几次艾特」，不是「最近几条消息」。
- 顶部会附一句摘要：一共多少条记录、最近一次是多久以前（`8 小时前`、`3 天前（09-03 21:40）`）。
- 想看上下文，需要群里开过一次 `开启艾特上下文`（群管 / 主人权限）。**没开过就只有艾特本身**，回顾会在摘要里提醒你这件事。
