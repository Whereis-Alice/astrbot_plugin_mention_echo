...
if __package__:
    ...
    from .modules.maintenance import MaintenanceMixin
...
class MentionEchoPlugin(
    ...
    """记录并回放群聊里的每一次艾特：查询长图、离线补发提醒、排行榜与存储自治。"""
    ...
    async def initialize(self) -> None:
        ...
        1. 把旧插件（谁艾特我 / 谁艾特我 Pro）的 KV 数据搬过来，做到装上就能用；
        2. 按配置决定是否向 LLM 暴露查询工具；
        3. 启动后台自动清理巡检。
        """
        try:
            migrated = await self._migrate_legacy_kv()
        except Exception as exc:
            logger.warning(f"{LOG_TAG} 旧数据迁移失败，将跳过: {exc}")
        else:
            if migrated:
                logger.info(f"{LOG_TAG} 已继承旧插件 {migrated} 条数据")

        if not self._llm_tool_enabled():
            try:
                self.context.deactivate_llm_tool(LLM_TOOL_NAME)
            except Exception as exc:
                logger.debug(f"{LOG_TAG} 停用 LLM 工具失败: {exc}")
    ...
    def _register_page_apis(self, context: Context) -> None:
        try:
            ...
            logger.warning(f"[艾特回声] Page API unavailable: {exc}")
            return
            ...
            for suffix, handler, methods, description in endpoints:
                ...
                try:
                    context.register_web_api(
                        ...
                        handler,
                        methods,
                        f"mention_echo {description}",
                    )
                except Exception as exc:
                    logger.warning(f"[艾特回声] Page API registration failed for {route}: {exc}")

    def _event_group_message_count(self, event: AstrMessageEvent, group_id: str) -> int:
        holder = getattr(event, "message_obj", event)
        cached = self._numeric_order(getattr(holder, "_mention_echo_group_message_count", None))
        if cached is not None:
            return cached
        count = self._group_message_counts.get(group_id, 0) + 1
        self._group_message_counts[group_id] = count
        try:
            setattr(holder, "_mention_echo_group_message_count", count)
        except Exception:
            pass
        return count
    ...
    async def mark_group_activity_early(self, event: AstrMessageEvent):
        ...
        try:
            setattr(event, "_mention_echo_activity_handled", True)
        except Exception:
            pass
        try:
            ...
            try:
                setattr(event, "_mention_echo_activity_handled", False)
            except Exception:
                pass
            logger.error(f"[艾特回声] early activity update failed: {exc}")
    ...
    async def on_group_message(self, event: AstrMessageEvent):
        ...
        activity_handled = bool(getattr(event, "_mention_echo_activity_handled", False))
    ...
    async def _handle_command(
        ...
        if STORAGE_PATTERN.match(stripped):
            ...
            return [event.plain_result(await self._storage_report_text())]

        if CLEANUP_PATTERN.match(stripped):
            if not self._is_bot_admin(event):
                return [event.plain_result("只有 AstrBot 管理员或主人可以手动触发清理")]
            report = await self.run_maintenance("manual")
            return [event.plain_result(self._maintenance_report_text(report))]
    ...
    async def _return_message_targets(self, event: AstrMessageEvent) -> set[str]:
        ...
        try:
            ...
            logger.warning(f"[艾特回声] failed to resolve return-message quote: {exc}")
            quote = None
    ...
    async def _deliver_pending_reminders(self, event: AstrMessageEvent, group_id: str, user_id: str) -> None:
        ...
        if self._render_text_only():
            ...
            if delivered:
                ...
                return
            logger.warning(
                f"[艾特回声] 合并转发发送失败（text 渲染模式），未向群内提示，"
            ...
            if reminder_text:
                if await self._try_send_text_images(event, reminder_text, image_paths):
                    ...
                    for image_path in image_paths:
                        if await self._try_send(event, event.image_result(image_path)):
                            ...
                            logger.warning(f"[艾特回声] 提醒图片回退发送失败: {image_path}")
                    if not delivered:
                ...
                for image_path in image_paths:
                    if await self._try_send(event, event.image_result(image_path)):
                        ...
                        logger.warning(f"[艾特回声] 提醒图片发送失败: {image_path}")
                if not delivered:
                    raise RuntimeError("failed to send reminder images")
            self._drop_records_image_cache(original_pending, delete_files=True)
        except Exception as exc:
            logger.error(f"[艾特回声] 渲染或发送提醒失败: {exc}")
            if not delivered:
                if reminder_text and not text_sent:
                    text_sent = await self._try_send(event, event.plain_result(reminder_text))
                delivered = await self._try_send_records_forward_text(
                    event,
                    pending,
                    target_name,
                    title="" if text_sent else reminder_text,
                )
                if not delivered:
                    logger.warning(
                        f"[艾特回声] 图片与合并转发均发送失败，未向群内提示，"
                        f"已保留 {len(original_pending)} 条待提醒记录: group={group_id} user={user_id}"
                    )
            if delivered:
                self._drop_records_image_cache(original_pending, delete_files=True)
    ...
    async def _help(self, event: AstrMessageEvent) -> list[Any]:
        ...
        try:
            ...
            logger.error(f"[艾特回声] render or send help image failed: {type(exc).__name__}: {exc}")
    ...
    def _help_sections(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                "title": "排行",
                "items": [
                    {"command": "艾特排行榜", "desc": "看看谁最常艾特你。"},
                    {"command": "艾特排行榜 7", "desc": "只统计最近 7 天，天数可自定义。"},
                ],
            },
            {
                "title": "维护",
                "items": [
                    {"command": "#艾特存储", "desc": "群管查看磁盘占用、记录条数与自动清理状态。"},
                    {"command": "#艾特清理", "desc": "主人立即跑一轮清理，不必等下次巡检。"},
                ],
            },
        ]
    ...
    async def _try_send_help_forward_text(self, event: AstrMessageEvent, sections: list[dict[str, Any]]) -> bool:
        ...
        nodes = [
            {
                ...
                "data": {
                    "name": bot_name,
                    "uin": uin,
                    "content": [{"type": "text", "data": {"text": "艾特回声 · 帮助"}}],
                },
            }
        ]
        for section in sections:
        ...
        if not sent:
            logger.warning("[艾特回声] records forward message unavailable; suppressing long plain-text fallback")
        return sent
    ...
    async def _try_send_records_forward_text(
        ...
        *,
        ...
        for msg in messages[:limit]:
            ...
            if message_id:
                data = {"id": message_id}
                timestamp = self._forward_node_time(msg)
                if timestamp > 0:
                    data["time"] = timestamp
            ...
            nodes.append(
                self._forward_text_node(
                    name,
                    uin,
                    self._message_forward_text(msg, target_name),
                    time_value=self._forward_node_time(msg),
                )
            )
        if len(messages) > limit:
            overflow_time = self._forward_node_time(messages[limit - 1]) if limit > 0 else 0
            nodes.append(
                self._forward_text_node(
                    bot_name,
                    bot_uin,
                    f"还有 {len(messages) - limit} 条消息未展示。",
                    time_value=overflow_time,
                )
            )
    ...
    def _forward_text_node(self, name: str, uin: Any, text: str, *, time_value: Any = None) -> dict[str, Any]:
        data = {
            "name": name,
            "uin": uin,
            "content": [{"type": "text", "data": {"text": text}}],
        }
        timestamp = self._forward_node_time(time_value)
        if timestamp > 0:
            data["time"] = timestamp
        return {
            "type": "node",
    ...
    def _forward_node_time(self, value: Any) -> int:
    ...
    async def _rank(self, event: AstrMessageEvent, group_id: str, match: re.Match[str]) -> list[Any]:
        ...
        if not me:
            return [event.plain_result("拿不到你的账号信息，无法统计")]
        ...
        if days > 0:
            ...
            records = [record for record in records if self._record_time(record) >= cutoff]
        scope_text = f"最近 {days} 天" if days else "目前"
        if not records:
            return [event.plain_result(f"{scope_text}还没有人艾特过你。")]
        ...
        for record in records:
            ...
            stamp = self._record_time(record)
            if stamp >= latest.get(user_id, 0):
                latest[user_id] = stamp
            name = str(record.get("name") or "").strip()
            if name and not self._looks_like_numeric_id(name):
                names[user_id] = name
        if not counts:
            return [event.plain_result("记录里没有可统计的艾特来源。")]

        ranking = sorted(counts.items(), key=lambda item: (-item[1], -latest.get(item[0], 0)))
        target_name = await self._target_name(event, group_id, me)
        medals = ("🥇", "🥈", "🥉")
        lines = [f"艾特 {target_name} 最多的人（{scope_text}）", ""]
        for index, (user_id, count) in enumerate(ranking[:RANK_TOP_N]):
            badge = medals[index] if index < len(medals) else f"{index + 1}."
            display = names.get(user_id) or user_id
            lines.append(f"{badge}  {display} · {count} 次 · 最近 {self._rank_time_text(latest.get(user_id, 0))}")
        lines.append("")
        lines.append(f"共 {len(records)} 条记录，来自 {len(counts)} 位成员")
    ...
    async def mention_echo_recent_mentions(self, event: AstrMessageEvent, limit: str = "5") -> str:
        """查询当前群聊里最近有哪些人艾特（@）了提问者，返回纯文本摘要。

        Args:
            limit(string): 最多返回的记录条数，默认 5，最大 20。
        """
        if not self._llm_tool_enabled():
            return "该工具已被管理员关闭。"
        group_id = self._group_id(event)
        me = self._sender_id(event)
        if not group_id or not me:
            return "该工具只能在群聊中使用。"

        try:
            count = int(str(limit or "5").strip() or "5")
        except ValueError:
            count = 5
        count = max(1, min(LLM_TOOL_MAX_RESULTS, count))

        records = self._dedupe_records(
            await self._get_records(group_id, me) + await self._get_records(group_id, ALL_TARGET)
        )
        if not records:
            return "最近没有人在本群艾特提问者。"

        records = sorted(records, key=self._record_time)[-count:]
        lines = []
        for record in reversed(records):
            when = self._rank_time_text(self._record_time(record))
            who = str(record.get("name") or record.get("user_id") or "未知")
            message = " ".join(str(record.get("message") or "").split())
            if len(message) > 80:
                message = message[:80] + "…"
            lines.append(f"{when} {who}：{message or '（无文字内容）'}")
        return "最近艾特提问者的记录（新到旧）：\n" + "\n".join(lines)
    ...
    async def _clear_all_locked(self, event: AstrMessageEvent) -> Any:
        ...
        # 成员名片缓存没有索引，只能全量枚举后清除，否则清完记录仍会留下一堆 member:* 键。
        removed_members = await self._purge_member_cache()
        ...
        summary = "已成功清除全部艾特数据"
        if removed_members:
            summary += f"（含 {removed_members} 条成员名片缓存）"
    ...
    async def terminate(self):
        ...
        self._group_message_counts.clear()
        self._kv_locks.clear()
