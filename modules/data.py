...
class DataMixin:
        ...
        async with self._preferences_write_lock():
            for attempt in range(len(delays) + 1):
                try:
                    ...
                    message = (
                        "[艾特回声] preferences database locked; "
    ...
    def _is_valid_dns_name(self, host: str) -> bool:
        text = str(host or "")
        if text.endswith("."):
            text = text[:-1]
        if not text or len(text.encode("utf-8", "ignore")) > 253:
            return False
        labels = text.split(".")
        return all(label and len(label.encode("utf-8", "ignore")) <= 63 for label in labels)
    ...
    def _is_allowed_remote_image_url(self, value: str) -> bool:
        ...
        try:
            ...
            if not self._is_valid_dns_name(host_text):
                return False
        try:
            infos = socket.getaddrinfo(host, None, type=socket.SOCK_STREAM)
        except (OSError, ValueError):
            # idna 编码超长 label 时抛 UnicodeError，它是 ValueError 而不是 OSError 的子类。
            return False
        for info in infos:
            try:
    ...
    async def _event_group(self, event: AstrMessageEvent, group_id: str) -> Any:
        ...
        try:
            ...
            logger.debug(f"[艾特回声] 获取群信息失败: {exc}")
            return None
