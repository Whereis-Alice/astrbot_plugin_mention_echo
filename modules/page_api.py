...
class PageApiMixin:
    async def page_layout(self):
        try:
            ...
            logger.error(f"[艾特回声] Page布局接口失败: {exc}", exc_info=True)
    ...
    async def page_fonts(self):
        try:
            ...
            logger.error(f"[艾特回声] Page字体列表失败: {exc}", exc_info=True)
    ...
    async def page_font_upload(self):
        try:
            ...
            logger.info(f"[艾特回声] Page上传并启用自定义字体: {target.name}")
            return self._json_response({"status": "ok", "message": "字体已上传并启用", "data": self._font_data()})
        except Exception as exc:
            logger.error(f"[艾特回声] Page上传字体失败: {exc}", exc_info=True)
    ...
    async def page_font_select(self):
        try:
            ...
            logger.info(f"[艾特回声] Page切换自定义字体: {font_path or '默认字体'}")
            return self._json_response({"status": "ok", "message": "字体设置已保存", "data": self._font_data()})
        except Exception as exc:
            logger.error(f"[艾特回声] Page选择字体失败: {exc}", exc_info=True)
    ...
    async def page_font_delete(self):
        try:
            ...
            logger.info(f"[艾特回声] Page删除自定义字体: {font_name}")
            return self._json_response({"status": "ok", "message": "字体已删除", "data": self._font_data()})
        except Exception as exc:
            logger.error(f"[艾特回声] Page删除字体失败: {exc}", exc_info=True)
    ...
    async def page_image_upload(self):
        try:
            ...
            logger.info(f"[艾特回声] Page上传并启用自定义{IMAGE_KINDS[kind]}: {target.name}")
            return self._json_response({"status": "ok", "message": "图片已上传并启用", "data": self._page_data()})
        except Exception as exc:
            logger.error(f"[艾特回声] Page上传图片失败: {exc}", exc_info=True)
    ...
    async def page_image_reset(self):
        try:
            ...
            logger.error(f"[艾特回声] Page恢复默认图片失败: {exc}", exc_info=True)
