...
class PageSettingsMixin:
    ...
    def _plugin_data_dir(self) -> Path:
        ...
        try:
            ...
            legacy_dirs = [Path(StarTools.get_data_dir(name)) for name in LEGACY_PLUGIN_NAMES]
        except Exception:
            try:
            ...
            legacy_dirs = [root / name for name in LEGACY_PLUGIN_NAMES]

        self._migrate_legacy_plugin_data(legacy_dirs, target)
        target.mkdir(parents=True, exist_ok=True)
        self._plugin_data_dir_cache = target
        return target

    def _migrate_legacy_plugin_data(self, legacy_dirs: Any, target: Path) -> None:
        """把上游插件的数据目录（字体、页面设置、自定义图片等）搬到新目录，只做一次。"""
        marker = target / LEGACY_DATA_MARKER
        if marker.exists():
            return
        if isinstance(legacy_dirs, (str, Path)):
            legacy_dirs = [Path(legacy_dirs)]
        migrated: list[str] = []
        try:
            target.mkdir(parents=True, exist_ok=True)
            ...
            for legacy in legacy_dirs:
                legacy = Path(legacy)
                if not legacy.exists() or legacy.resolve() == resolved_target:
                    continue
            ...
            marker.write_text(",".join(migrated), encoding="utf-8")
            if migrated:
                logger.info(f"[艾特回声] migrated legacy plugin data from {migrated} to {target}")
        except Exception as exc:
            logger.warning(f"[艾特回声] legacy data migration failed: {exc}")
    ...
    def _load_page_settings(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"[艾特回声] 读取 Page 设置失败，使用默认值: {exc}")
    ...
    def _resolve_custom_font_path(self) -> Path | None:
        ...
        for candidate in candidates:
            try:
                ...
                continue
        logger.warning(f"[艾特回声] 自定义字体文件不存在，使用默认字体: {font_path}")
        return None
    ...
    def _custom_font_css(self) -> str:
        ...
        try:
            ...
            logger.warning(f"[艾特回声] 读取自定义字体失败，使用默认字体: {exc}")
            self._font_css_cache_key = None
            self._font_css_cache_value = ""
            return ""
        ...
        css = (
            "@font-face { "
            "font-family: 'MentionEchoCustomFont'; "
            f"src: url(\"data:{self._font_mime_type(font_path)};base64,{font_data}\") format('{self._font_format(font_path)}'); "
            "font-weight: 100 900; font-style: normal; font-display: block; "
            "}\n"
            ":root { --mention-echo-font-family: 'MentionEchoCustomFont', 'Microsoft YaHei', 'Segoe UI', sans-serif; }\n"
            "body, body * { font-family: var(--mention-echo-font-family) !important; }"
        )
    ...
    def _page_font_css(self) -> str:
        ...
        try:
            ...
            logger.warning(f"[艾特回声] 读取 Page 预览字体失败，使用默认字体: {exc}")
            return ""
        return (
            "@font-face { "
            "font-family: 'MentionEchoPreviewFont'; "
            f"src: url(\"data:{self._font_mime_type(font_path)};base64,{font_data}\") format('{self._font_format(font_path)}'); "
            "font-weight: 100 900; font-style: normal; font-display: block; "
            "}\n"
            ".phone, .phone * { font-family: 'MentionEchoPreviewFont', 'Microsoft YaHei', 'Segoe UI', sans-serif !important; }"
        )
    ...
    def _image_file_data_url(self, path: Path) -> str:
        try:
            ...
            logger.warning(f"[艾特回声] 读取本地图片失败: {path} {exc}")
            return ""
